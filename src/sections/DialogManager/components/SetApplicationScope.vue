<template>
  <base-dialog @cancel="cancelDialog">
    <div slot="header">{{ $t('bill.budget_scope_label') }}</div>
    <div slot="body">
      <dialog-selected-tips :count="params.data.length" :name="params.name || $t('common_92')" :action="$t('bill.budget_scope_label')" />
      ...
      <a-form-model
        ...
        <a-form-model-item :label="$t('bill.budget_scope_label')" prop="type">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        type: [
          { required: true, message: this.$t('common.tips.select', [this.$t('bill.budget_scope_label')]) },
        ],
        shared_domains: [
