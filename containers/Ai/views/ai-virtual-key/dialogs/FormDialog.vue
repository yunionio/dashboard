  ...
  <base-dialog :width="760" @cancel="cancelDialog">
    ...
    <div slot="body">
      <a-form-model ref="form" :model="form" :rules="rules" :label-col="{ span: 5 }" :wrapper-col="{ span: 17 }">
        ...
        <a-form-model-item :label="$t('aice.aiproxy.virtual_key_field')" prop="virtual_key">
        ...
        <a-form-model-item :label="$t('aice.aiproxy.allowed_ai_provider_ids')">
          <base-select
            v-model="form.allowed_ai_provider_ids"
...
export default {
  ...
  data () {
    ...
    return {
      ...
      form: {
        ...
        allowed_ai_provider_ids: data.limits?.allowed_ai_provider_ids || [],
      },
      rules: {
  ...
  methods: {
    ...
    buildPayload () {
      ...
      if (this.form.virtual_key) data.virtual_key = this.form.virtual_key
