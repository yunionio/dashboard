  ...
  <base-dialog @cancel="cancelDialog">
    ...
    <div slot="body">
      ...
      <a-form :form="form.fc" hideRequiredMark v-bind="formItemLayout">
        <a-form-item :label="$t('compute.nic')">
          <base-select
            class="w-100"
            v-decorator="decorators.nic"
            :options="nicOptions"
            :select-props="{ allowClear: false, placeholder: $t('compute.text_193') }"
            @change="handleNicChange" />
        </a-form-item>
        <a-form-item :label="$t('compute.text_105')" v-if="selectedNic">
...
export default {
  name: 'VmSetNetworkSecgroupDialog',
      ...
      decorators: {
        nic: [
          'nic',
          {
            initialValue: null,
            rules: [
              { required: true, message: this.$t('compute.text_193') },
            ],
          },
        ],
        secgroups: [
  ...
  created () {
    this.initNicOptions()
    this.$watch(() => this.form.fc.getFieldValue('nic'), (val) => {
      if (val) {
        this.fetchBindedSecgroups()
      } else {
        this.bindedSecgroupsLoaded = false
  ...
  methods: {
    initNicOptions () {
      ...
      // 默认选择第一个网卡
      if (this.nicOptions.length > 0) {
        this.$nextTick(() => {
          const firstNicValue = this.nicOptions[0].value
          this.form.fd.nic = firstNicValue
          this.form.fc.setFieldsValue({ nic: firstNicValue })
          // 触发安全组加载
          this.fetchBindedSecgroups()
        })
      }
