...
export default {
  name: 'K8SFederatedclusterrolebindingClusterList',
  mixins: [WindowsMixin],
  props: {
    data: {
      type: Object,
  ...
  data () {
    return {
      ...
      groupActions: [
        {
          ...
          permission: 'k8s_federatedclusterrolebindings_perform_attach_cluster',
          action: () => {
      ...
      singleActions: [
        {
          label: '解绑',
          permission: 'k8s_federatedclusterrolebindings_perform_detach_cluster',
          action: (obj) => {
  ...
  methods: {
    ...
    async fetchData (params) {
      try {
        ...
        const res = await new this.$Manager('federatedclusterrolebindings', 'v1').getSpecific(managerArgs)
        return res
      } catch (error) {
        throw error
      }
