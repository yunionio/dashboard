...
export default {
  name: 'K8SFederatedrolebindingClusterList',
  mixins: [WindowsMixin],
  props: {
    data: {
      type: Object,
  ...
  data () {
    return {
      ...
      groupActions: [
        {
          ...
          permission: 'k8s_federatedrolebindings_perform_attach_cluster',
          action: () => {
      ...
      singleActions: [
        {
          label: '解绑',
          permission: 'k8s_federatedrolebindings_perform_detach_cluster',
          action: (obj) => {
  ...
  methods: {
    ...
    async fetchData (params) {
      try {
        ...
        const res = await new this.$Manager('federatedrolebindings', 'v1').getSpecific(managerArgs)
        return res
      } catch (error) {
        throw error
      }
