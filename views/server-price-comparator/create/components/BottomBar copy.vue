...
export default {
  ...
  computed: {
    ...
    region () {
      let ret = this.fd.cloudregion ? this.fd.cloudregion.label : ''
      if (this.isPublic) {
        ret = this.fd.sku ? this.fd.sku.region : ''
      }
      return ret
    },
    zone () {
    ...
    config () {
      const { sku: { name, instance_type_category_i18n, memory_size_mb, memory_size_mb_compute } } = this.fd
      return `${name}(${instance_type_category_i18n}${memory_size_mb_compute}${this.$t('common_60')}${sizestr(memory_size_mb, 'M', 1024)})`
    },
    systemDisk () {
      let diskStr = ''
      if (this.fd.systemDiskSize) diskStr = `${this.fd.systemDiskSize}GB ${_.get(this.fd, 'systemDiskType.label')}`
    ...
    count () {
      return `${this.fd.count}${this.$t('common_62')}`
    },
    tips () {
      const ret = [
        [
        ...
        [
          { label: '计费模式', labelClass: 'label-w-80', value: this.isPackage ? '包年包月' : '按量付费' },
          { label: '购买量', labelClass: 'label-w-80', value: this.count },
        ],
        [
          { label: this.$t('compute.text_177'), labelClass: 'label-w-50', value: this.region },
          { label: this.$t('compute.text_49'), labelClass: 'label-w-50', value: this.systemDisk },
        ],
      ]
      return ret
    },
