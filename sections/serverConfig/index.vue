  ...
  <div>
    <a-card class="mb-2 card" hoverable v-for="(item, i) in serverConfigList" :key="item.key">
      <a-form-item label="CPU" v-bind="formItemLayout">
        <a-input-number v-decorator="decorator.vcpu_count(i)" :formatter="value => `${value}核`" :parser="value => value.replace('核', '')" />
      </a-form-item>
      <a-form-item label="内存" v-bind="formItemLayout">
        <a-input-number v-decorator="decorator.vmem_size(i)" :formatter="value => `${value}G`" :parser="value => value.replace('G', '')" />
      </a-form-item>
